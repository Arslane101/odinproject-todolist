...
function createLayout(task) {
    ...
    if (checkbox.checked) {
      styles.insertRule(
        ":checked { background-color:" +
          colors[4 - parseInt(task.priority.at(1))] +
          "}",
      );
      ...
      taskspace.addEventListener(
        "transitionend",
        () => {
          taskspace.remove();
          tasklist.splice(
            tasklist.findIndex((item) => item.getID() === task.getID()),
            1,
          );
  ...
  checkbox.style.border =
    "2px solid " + colors[4 - parseInt(task.priority.at(1))];
